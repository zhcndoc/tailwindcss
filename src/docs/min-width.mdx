...
export const description = "设置元素最小宽度的工具类.";
...
使用 `min-w-<number>` 工具类，如 `min-w-24` 和 `min-w-64`，根据间距比例设置元素的固定最小宽度：
...
使用 `min-w-full` 或 `min-w-<fraction>` 工具类，如 `min-w-1/2` 和 `min-w-2/5`，给元素提供基于百分比的最小宽度：
...
使用工具类，如 `min-w-sm` 和 `min-w-xl`，根据容器比例设置元素的固定最小宽度：
