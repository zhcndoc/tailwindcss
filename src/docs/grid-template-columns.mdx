...
export const description = "用于指定网格布局中的列的实用工具。";
...
使用 `grid-cols-<number>` 实用工具，例如 `grid-cols-2` 和 `grid-cols-4` 来创建具有 _n_ 个相同大小列的网格：
...
使用 `grid-cols-subgrid` 实用工具来采用由项目的父元素定义的列轨道：
