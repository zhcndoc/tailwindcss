...
export const description = "用于设置元素最小内联尺寸的实用工具。";
...
使用 `min-inline-<number>` 实用工具，比如 `min-inline-24` 和 `min-inline-64`，基于间距刻度为元素设置固定的最小内联尺寸：
...
使用 `min-inline-full` 或 `min-inline-<fraction>` 实用工具，例如 `min-inline-1/2` 和 `min-inline-2/5`，为元素设置基于百分比的最小内联尺寸：
...
使用 `min-inline-sm` 和 `min-inline-xl` 等实用工具，根据容器刻度为元素设置固定的最小内联尺寸：
...
<UsingACustomValue utility="min-inline" value="220px" name="最小内联尺寸" variable="min-inline-size" />
