...
export const description = "控制定位元素放置的工具类。";
...
要使用负的 top/right/bottom/left 值，请在类名前加上一个负号以转换为负值：
...
使用 `start-<number>` 或 `end-<number>` 工具类，如 `start-0` 和 `end-4` 来设置 `inset-inline-start` 和 `inset-inline-end` 的 [逻辑属性](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts)，这些属性根据文本方向映射到左侧或右侧：
...
为了更好的控制，你还可以使用 [LTR 和 RTL 修饰符](/docs/hover-focus-and-other-states#rtl-support) 来根据当前文本方向有条件地应用特定样式。
