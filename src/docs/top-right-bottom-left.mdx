...
使用 `inset-s-<number>` 或 `inset-e-<number>` 工具类，如 `inset-s-0` 和 `inset-e-4` 来设置 `inset-inline-start` 和 `inset-inline-end` 的 [逻辑属性](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts)，这些属性根据文本方向映射到左侧或右侧：
