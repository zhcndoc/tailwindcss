...
export const description = "用于控制 flex 项目如何增长和缩小的工具。";
...
使用 `flex-<number>` 工具，例如 `flex-1`，使 flex 项目根据需要生长和缩小，忽略其初始大小：
...
使用 `flex-initial` 使 flex 项目可以缩小但不可以增长，考虑其初始大小：
...
使用 `flex-auto` 使 flex 项目可增长和缩小，考虑其初始大小：
