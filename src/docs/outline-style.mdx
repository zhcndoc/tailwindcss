...
export const description = "用于控制元素轮廓样式的工具类。";
...
使用 `outline-solid` 和 `outline-dashed` 等工具类设置元素的轮廓样式：
...
使用 `outline-hidden` 工具类在获得焦点的元素上隐藏默认的浏览器轮廓，同时在强制颜色模式下保留轮廓：
...
强烈建议在使用此工具类时应用自己的聚焦样式以提高可访问性。

### 移除轮廓

使用 `outline-none` 工具类完全移除获得焦点的元素上的默认浏览器轮廓：
...
<Example>
  {
    <div className="mx-auto flex max-w-md flex-col rounded-lg outline-1 outline-gray-300 focus-within:outline-2 focus-within:outline-indigo-600 dark:bg-white/5 dark:outline-transparent dark:focus-within:outline-indigo-500">
      <textarea className="w-full resize-none p-2 outline-none" placeholder="留下您的评论..." />
      <button
...
<div class="focus-within:outline-2 focus-within:outline-indigo-600 ...">
  <textarea class="outline-none ..." placeholder="留下您的评论..." />
