...
export const description = "控制 SVG 蒙版解释方式的工具。";
...
使用 `mask-type-alpha` 和 `mask-type-luminance` 工具来控制 SVG 蒙版的类型：
...
使用 `mask-type-luminance` 时，SVG 蒙版的亮度值决定了可见性，因此使用灰度颜色将产生最可靠的结果。而使用 `mask-alpha` 时，SVG 蒙版的不透明度决定了被蒙版元素的可见性。
