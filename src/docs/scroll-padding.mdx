...
export const description = "用于控制元素在快照容器内的滚动偏移的工具类。";
...
使用 `scroll-pt-<number>`, `scroll-pr-<number>`, `scroll-pb-<number>`, 和 `scroll-pl-<number>` 工具类，例如 `scroll-pl-4` 和 `scroll-pt-6`，设置元素在快照容器内的滚动偏移：
...
使用 `scroll-ps-<number>` 和 `scroll-pe-<number>` 工具类设置 `scroll-padding-inline-start` 和 `scroll-padding-inline-end` 逻辑属性，这些属性根据文本方向映射到左或右侧：
...
要使用负的滚动填充值，请在类名前面加上一个短横线以将其转换为负值：
