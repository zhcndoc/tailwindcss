...
使用 `translate-<number>` 工具类，如 `translate-2` 和 `-translate-4`，根据间距比例在两个轴上平移元素：
...
使用 `translate-<fraction>` 工具类，如 `translate-1/4` 和 `-translate-full`，按照元素大小的百分比在两个轴上平移元素：
...
使用 `translate-x-<number>` 或 `translate-x-<fraction>` 工具类，如 `translate-x-4` 和 `translate-x-1/4`，在 x 轴上平移一个元素：
...
使用 `translate-y-<number>` 或 `translate-y-<fraction>` 工具类，如 `translate-y-6` 和 `translate-y-1/3`，在 y 轴上平移一个元素：
...
使用 `translate-z-<number>` 工具类，如 `translate-z-6` 和 `-translate-z-12`，在 z 轴上平移一个元素：
...
注意 `translate-z-<number>` 工具类需要将 `transform-3d` 工具应用于父元素。
