...
如果你只是使用默认主题，没有进行任何自定义（例如没有使用 `@theme`、`@custom-variant`、`@plugin` 等），你可以直接导入 `tailwindcss`：
...
在使用 CLI、Vite 或 PostCSS 时，指令 `@import`、`@reference`、`@plugin` 和 `@config` 都支持[子路径导入](https://nodejs.org/api/packages.html#subpath-imports)，其工作方式类似于打包工具和 TypeScript 路径别名：
