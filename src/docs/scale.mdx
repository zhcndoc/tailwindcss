...
使用 `scale-<number>` 工具类，例如 `scale-75` 和 `scale-150`，按原始尺寸的百分比缩放元素：
...
使用 `scale-x-<number>` 工具类，例如 `scale-x-75` 和 `-scale-x-150`，按原始宽度的百分比在 x 轴上缩放元素：
...
使用 `scale-y-<number>` 工具类，例如 `scale-y-75` 和 `scale-y-150`，按原始高度的百分比在 y 轴上缩放元素：
...
使用 `-scale-<number>`、`-scale-x-<number>` 或 `-scale-y-<number>` 工具类，例如 `-scale-x-75` 和 `-scale-125`，按原始尺寸的百分比镜像并缩放元素：
