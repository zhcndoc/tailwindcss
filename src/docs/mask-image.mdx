...
export const description = "用于控制元素遮罩图像的工具类。";
...
使用像 `mask-b-from-<value>` 和 `mask-t-to-<value>` 这样的工具类，为元素的单侧添加线性渐变遮罩：
...
此外，使用像 `mask-x-from-70%` 和 `mask-y-to-90%` 这样的工具类，可以同时对元素的两个侧面应用遮罩：
...
默认情况下，线性渐变遮罩从黑色过渡到透明，但你可以使用 `mask-<side>-from-<color>` 和 `mask-<side>-to-<color>` 工具类自定义渐变颜色。

### 添加斜线性遮罩

使用像 `mask-linear-<angle>`、`mask-linear-from-20` 和 `mask-linear-to-40` 这样的工具类，为元素添加自定义线性渐变遮罩：
...
使用 `mask-radial-from-<value>` 和 `mask-radial-to-<value>` 工具类，为元素添加径向渐变遮罩：
...
<Example padding={false}>
  {
    <div className="mx-auto flex items-center p-16 max-sm:p-6">
      ...
      <div className="font-medium max-sm:-mx-3">
        ...
        <p className="mt-2 text-base whitespace-nowrap text-gray-700 dark:text-gray-300">为强大用户而设计</p>
...
<div class="flex items-center gap-4">
  ...
  <div class="font-medium">
    ...
    <p class="mt-2 text-base text-gray-700 dark:text-gray-300">为强大用户而设计</p>
...
默认情况下，径向渐变遮罩从黑色过渡到透明，但你可以使用 `mask-radial-from-<color>` 和 `mask-radial-to-<color>` 工具类自定义渐变颜色。

#### 设置径向位置

使用像 `mask-radial-at-bottom-left` 和 `mask-radial-at-[35%_35%]` 这样的工具类，设置径向渐变遮罩的中心位置：
...
这与 [`mask-position`](/docs/mask-position) 不同，后者设置的是遮罩图像本身的位置，而非径向渐变。

#### 设置径向大小

使用像 `mask-radial-closest-corner` 和 `mask-radial-farthest-side` 这样的工具类设置径向渐变遮罩的大小：
...
设置自定义的径向渐变大小时，可以使用的单位取决于渐变的 `<ending-shape>`，默认设置为 `ellipse`。

使用 `mask-circle` 时，只能使用单个固定长度，例如 `mask-radial-[5rem]`。而使用 `mask-ellipse`，则可以为每个轴指定固定长度或百分比，例如 `mask-radial-[40%_80%]`。

### 添加锥形遮罩

使用 `mask-conic-from-<value>`、`mask-conic-to-<value>` 和 `mask-conic-<angle>` 工具类，为元素添加锥形渐变遮罩：
...
<Example>
  {
    <div className="mx-auto flex w-full max-w-sm items-center gap-5 rounded-xl bg-white p-4 shadow-lg ring-1 ring-black/5 dark:bg-gray-800">
      ...
      <div className="w-0 flex-1 text-sm text-gray-950 dark:text-white">
        <p className="font-medium">已用存储：75%</p>
        <p className="mt-1 text-gray-500 dark:text-gray-400">
          <span className="font-medium">0.48 GB</span> 剩余 2 GB
        </p>
      </div>
    </div>
  }
...
<div class="flex items-center gap-5 rounded-xl bg-white p-4 shadow-lg ring-1 ring-black/5 dark:bg-gray-800">
  ...
  <div class="w-0 flex-1 text-sm text-gray-950 dark:text-white">
    <p class="font-medium">已用存储：75%</p>
    <p class="mt-1 text-gray-500 dark:text-gray-400"><span class="font-medium">0.48 GB</span> 剩余 2 GB</p>
  </div>
</div>
```
...
默认情况下，锥形渐变遮罩从黑色过渡到透明，但你可以使用 `mask-conic-from-<color>` 和 `mask-conic-to-<color>` 工具类自定义渐变颜色。

### 组合遮罩

渐变遮罩工具类，如 `mask-radial-from-<value>`、`mask-conic-to-<value>` 和 `mask-l-from-<value>` 可以组合使用，以创建更复杂的渐变遮罩：
...
这种行为依赖于 Tailwind 默认将 [`mask-composite` 属性](/docs/mask-composite) 设置为 `intersect`。更改此属性将影响渐变遮罩的组合方式。

### 移除遮罩图像

使用 `mask-none` 工具类从元素中移除现有的遮罩图像：
