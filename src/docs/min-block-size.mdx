...
export const description = "用于设置元素最小块尺寸的工具类。";
...
使用 `min-block-<number>` 工具类，例如 `min-block-24` 和 `min-block-64`，根据间距刻度为元素设置固定的最小块尺寸：
...
使用 `min-block-full` 或 `min-block-<fraction>` 工具类，例如 `min-block-1/2` 和 `min-block-2/5`，为元素设置基于百分比的最小块尺寸：
...
<UsingACustomValue utility="min-block" value="220px" name="最小块尺寸" variable="min-block-size" />
