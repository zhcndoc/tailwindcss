...
使用 `h-<数字>` 工具类，如 `h-24` 和 `h-64`，根据间距比例设置元素的固定高度：
...
使用 `h-full` 或 `h-<分数>` 工具类，如 `h-1/2` 和 `h-2/5`，给元素设置基于百分比的高度：
...
使用 `h-dvh` 工具，使一个元素的高度覆盖动态变化的视口高度：

<Figure hint="滚动视口以查看视口高度的变化">
...
<UsingACustomValue utility="h" value="32rem" name="高度" variable="height" />
