...
export const title = "悬停、聚焦及其他状态";
export const description = "使用工具类为悬停、聚焦等状态的元素进行样式设置。";

Tailwind 中的每个工具类都可以通过在类名开头添加描述目标条件的变体来进行 _条件_ 应用。

例如，为了在悬停时应用 `bg-sky-700` 类，可以使用 `hover:bg-sky-700` 类：

<Figure hint="悬停在此按钮上以查看背景颜色变化">
...
<summary className="font-medium cursor-default select-none text-gray-900 dark:text-gray-200">这与传统 CSS 有什么不同？</summary>

在传统书写 CSS 时，单个类名会根据当前状态应用不同的样式：

<TipBad>{<>传统上，使用同一类名在悬停时应用不同的样式</>}</TipBad>
...
在 Tailwind 中，而不是将悬停状态的样式添加到现有类中，而是向元素添加另一个类，_仅_ 在悬停时执行某些操作：

<TipGood>{<>在 Tailwind 中，默认状态和悬停状态使用不同的类</>}</TipGood>
...
注意 `hover:bg-sky-700` _仅_ 定义了 `:hover` 状态的样式？它默认不做任何事情，但一旦您悬停在具有该类的元素上，背景颜色将会变为 `sky-700`。

这就是我们所说的工具类可以 _条件_ 应用的意思——通过使用变体，您可以精确控制设计在不同状态下的行为，而无需离开 HTML。

</details>

Tailwind 包含了几乎所有您可能需要的变体，包括：

- [伪类](#pseudo-classes)，如 `:hover`、`:focus`、`:first-child` 和 `:required`
- [伪元素](#pseudo-elements)，如 `::before`、`::after`、`::placeholder` 和 `::selection`
- [媒体和特性查询](#media-and-feature-queries)，如响应式断点、暗模式和 `prefers-reduced-motion`
- [属性选择器](#attribute-selectors)，如 `[dir="rtl"]` 和 `[open]`
- [子选择器](#child-selectors)，如 `& > *` 和 `& *`

这些变体甚至可以叠加使用，以应对更特定的情况，例如在暗模式下、在中等断点下、在悬停时更改背景颜色：
...
在本指南中，您将了解框架中可用的每一个变体、如何将它们与您自己的自定义类一起使用，甚至如何创建自己的变体。
...
<Figure hint="尝试与此按钮互动以查看悬停、聚焦和活动状态">
...
Tailwind 也包括其他交互状态的变体，如 `:visited`、`:focus-within`、`:focus-visible` 等等。

请参见 [伪类参考](#pseudo-class-reference) 获取可用的伪类变体的完整列表。
...
您也可以使用 `odd` 和 `even` 变体为奇数和偶数子元素设置样式：
...
使用 `nth-*` 和 `nth-last-*` 变体可以根据其在列表中的位置为子元素设置样式：
...
您可以默认传递任何数字，并使用任意值来进行更复杂的表达式，例如 `nth-[2n+1_of_li]`。
...
<Figure hint="尝试使电子邮件地址有效以查看样式变化">
...
使用这种类型的变体可以减少模板中的条件逻辑，从而让您可以无论输入处于何种状态都使用同一组类，并让浏览器为您应用正确的样式。
...
您可以将 `has-*` 与伪类结合使用，例如 `has-[:focus]`，根据后代的状态为元素设置样式。您也可以使用元素选择器，例如 `has-[img]` 或 `has-[a]`，根据后代的内容为元素设置样式。
...
如果您需要根据一个兄弟元素的后代为元素设置样式，可以使用 `peer` 类标记该兄弟，并使用 `peer-has-*` 变体为目标元素设置样式：
...
您还可以将 `not-` 变体与媒体查询变体（如 `forced-colors` 或 `supports`）结合使用，仅在用户环境中的某些条件不成立时为元素设定样式：
...
当您需要根据某个 _父_ 元素的状态为元素设置样式时，将父元素标记为 `group` 类，并使用 `group-*` 变体（如 `group-hover`）为目标元素设置样式：

<Figure hint="悬停在卡片上以查看两种文本元素的颜色变化">
...
这种模式适用于每种伪类变体，例如 `group-focus`、`group-active` 或甚至 `group-odd`。

#### 区分嵌套组

在嵌套组时，您可以通过使用 `group/{name}` 类给特定父组独特的组名，从而根据 _特定_ 父组的状态进行样式设置，并在变体中使用像 `group-hover/{name}` 的类中包括该名称：
...
组的命名方式可以是您喜欢的，并且不需要以任何方式进行配置——只需直接在标记中命名您的组，Tailwind 会自动生成必要的 CSS。

#### 任意组

您可以通过在方括号中提供自己的选择器作为 [任意值](/docs/adding-custom-styles#using-arbitrary-values) 动态创建一次性的 `group-*` 变体：
...
若需要更多控制，您可以使用 `&` 字符标记 `.group` 与您所传入选择器之间的最终选择器相对位置：
...
`in-*` 变体的工作方式与 `group` 类似，只是您不需要在父元素上添加 `group`：
...
`in-*` 变体响应于任何父元素的状态变化，因此，如果您需要更细粒度的控制，您需要使用 `group`。

### 基于兄弟状态进行样式设置

当您需要根据 _兄弟_ 元素的状态为元素设置样式时，可以通过将兄弟标记为 `peer` 类，并使用 `peer-*` 变体（如 `peer-invalid`）为目标元素设置样式：

<Figure hint="尝试使电子邮件地址有效以查看警告消失">
...
这使得可以进行各种有趣的操作，例如 [浮动标签](https://www.youtube.com/watch?v=nJzKi6oIvBA) 而不需要任何 JS。

这种模式适用于每种伪类变体，例如 `peer-focus`、`peer-required` 和 `peer-disabled`。

重要的是要注意，因为 [后续兄弟组合器](https://developer.mozilla.org/en-US/docs/Web/CSS/Subsequent-sibling_combinator) 的工作方式，`peer` 标记只能用于 _先前_ 的兄弟：

<TipBad>{<>不适用，只有前面的兄弟可以标记为同伴</>}</TipBad>

```html
<!-- [!code classes:peer-invalid:text-red-500] -->
<!-- [!code classes:peer] -->
<label>
  <span class="peer-invalid:text-red-500 ...">电子邮件</span>
  <input type="email" class="peer ..." />
</label>
```
...
使用多个同伴时，您可以通过给特定同伴一个唯一名称，使用 `peer/{name}` 类根据 _特定_ 同伴的状态进行样式设置并在变体中包括该名称，如 `peer-checked/{name}` 的类：
...
同伴可以随您喜欢的方式命名，并且不需要以任何方式进行配置——只需直接在标记中命名您的同伴，Tailwind 就会自动生成必要的 CSS。

#### 任意同伴

您可以通过在方括号中提供自己的选择器作为 [任意值](/docs/adding-custom-styles#using-arbitrary-values) 动态创建一次性的 `peer-*` 变体：
...
若需要更多控制，可以使用 `&` 字符标记 `.peer` 在您传入选择器中的最终选择器位置：
...
使用 `before` 和 `after` 变体为 `::before` 和 `::after` 伪元素设置样式：
...
<label>
  <span class="text-gray-700 after:ml-0.5 after:text-red-500 after:content-['*'] ...">电子邮件</span>
...
使用这些变体时，Tailwind 将默认自动添加 `content: ''`，因此除非您想要其他值，否则您不必指定：
      ...
      <div className="mx-auto max-w-lg border-x border-x-gray-200 px-6 py-8 dark:border-x-gray-800 dark:bg-gray-950/10">
        <blockquote className="text-center text-2xl font-semibold text-gray-900 italic dark:text-white">
          ...
          <span className="relative inline-block before:absolute before:-inset-1 before:block before:-skew-y-3 before:bg-pink-500">
            <span className="relative text-white dark:text-gray-950">烦恼</span>
          </span>{" "}
          时，人们会觉得你忙得不可开交。
        </blockquote>
      </div>
...
<blockquote class="text-center text-2xl font-semibold text-gray-900 italic dark:text-white">
  ...
  <span class="relative inline-block before:absolute before:-inset-1 before:block before:-skew-y-3 before:bg-pink-500">
    <span class="relative text-white dark:text-gray-950">烦恼</span>
  </span>
  时，人们会觉得你忙得不可开交。
</blockquote>
```
...
值得注意的是，在大多数 Tailwind 项目中，您实际上并不需要 `::before` 和 `::after` 伪元素——通常直接使用真实的 HTML 元素会更简单。
...
<blockquote class="text-center text-2xl font-semibold text-gray-900 italic">
  ...
  <span class="relative">
    ...
    <span class="relative text-white">烦恼</span>
  </span>
  时，人们会觉得你忙得不可开交。
</blockquote>
```

将 `before` 和 `after` 保留用于那些内容重要到伪元素内容不实际上存在于 DOM 中并且无法被用户选择的情况。
...
我们设计的 `marker` 变体是可继承的，因此尽管您可以直接在 `<li>` 元素上使用它，但您还可以在父元素上使用它，以避免冗余的样式。

### ::selection

使用 `selection` 变体为活动文本选择设置样式：

<Figure hint="尝试用鼠标选择一些文本">
...
我们设计的 `selection` 变体是可继承的，因此您可以在树中的任何位置添加它，并且它将应用于所有后代元素。
...
使用 `first-line` 变体为内容块的第一行设置样式，使用 `first-letter` 变体为第一字符设置样式：
...
使用 `backdrop` 变体为 [原生 `<dialog>` 元素](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/dialog) 的背景设置样式：
...
如果您在项目中使用原生 `<dialog>` 元素，您可能还想阅读关于 [样式开/关状态](/docs/hover-focus-and-other-states#openclosed-state) 的内容，使用 `open` 变体。
...
要基于父元素的宽度而不是视口来设置元素的样式，可以使用 `@md` 和 `@lg` 等变量：
...
查看 [响应式设计](/docs/responsive-design) 文档以深入了解这些功能的工作原理。
...
查看 [暗模式](/docs/dark-mode) 文档以深入了解此功能的工作原理。
...
<Figure hint="尝试在开发工具中模拟 &#96;prefers-reduced-motion: reduce&#96; 来隐藏旋转器">
...
Tailwind 还包括一个 `motion-safe` 变体，仅在用户 _没有_ 请求减少运动时添加样式。当使用 `motion-reduce` 帮助程序意味着必须“撤消”很多样式时，这可能非常有用：
...
Tailwind 还包括一个 `contrast-less` 变体，您可以在用户请求较少对比度时使用该变体条件性地添加样式。
...
<Figure hint="尝试在开发工具中模拟 &#96;forced-colors: active&#96; 来查看变化">
...
使用 `not-forced-colors` 变体在用户 _不_ 使用强制颜色模式时应用样式：
...
Tailwind 还提供了 [forced color adjust](/docs/forced-color-adjust) 工具，允许您选择加入或退出强制颜色。
...
<div>
  <article class="print:hidden">
    <h1>我的秘密披萨食谱</h1>
    <p>这个食谱是秘密，不能与任何人分享</p>
    <!-- ... -->
  </article>
  <div class="hidden print:block">你真的想打印这东西吗？它是秘密！</div>
</div>
```

### @supports

使用 `supports-[...]` 变体基于用户的浏览器是否支持某一特性来为元素设置样式：
...
在内部，`supports-[...]` 变体生成 [`@supports 规则`](https://developer.mozilla.org/en-US/docs/Web/CSS/@supports) 并将您可以与 `@supports (...)` 使用的内容放在方括号之间，例如属性/值对，甚至使用 `and` 和 `or` 的表达式。
...
使用 `not-supports-[...]` 变体根据用户的浏览器是否不支持某一特性为元素设置样式：
...
您可以通过在 `supports-*` 命名空间中创建新变体来为项目中使用的常见 `@supports` 规则配置快捷方式：
...
使用 `starting` 变体设置元素在首次渲染到 DOM 时或从 `display: none` 过渡到可见时的外观：
      ...
      <div className="grid h-full items-center justify-center">
        ...
        >
          ...
          <div className="mt-5">
            <button
              type="button"
              className="inline-flex w-full justify-center rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500 focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-indigo-600 sm:col-start-2"
            >
              Install
            </button>
          </div>
...
使用 `aria-*` 变体根据 [ARIA 属性](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Attributes) 条件性地设置样式。
...
如果您需要使用一次性 `aria` 变体，或者需要特定值的复杂 ARIA 属性，请使用方括号通过任何任意值动态生成属性：
...
ARIA 状态变体还可使用 `group-aria-*` 和 `peer-aria-*` 变体以目标父元素和兄弟元素：
...
使用 `data-*` 变体根据 [数据属性](https://developer.mozilla.org/en-US/docs/Learn/HTML/Howto/Use_data_attributes) 条件性地应用样式。
...
或者，您可以通过在 `data-*` 命名空间中创建新变体为项目中使用的常见数据属性配置快捷方式：
...
使用 `rtl` 和 `ltr` 变体在构建多方向布局时分别条件性地添加样式以支持从右到左和从左到右的模式：
...
请记住，如果您正在构建仅需支持 _一种_ 方向的站点，则这些变体是没有用的——只需应用适合您内容的样式。

### 开/关状态

使用 `open` 变体在 `<details>` 或 `<dialog>` 元素处于打开状态时条件性地添加样式：

<Figure hint="尝试切换披露以查看样式变化">
...
该变体还可以针对弹出窗口的 `:popover-open` 伪类：

```html
<!-- [!code classes:open:opacity-100] -->
<div>
  <button popovertarget="my-popover">打开弹出窗口</button>
...
`inert` 变体允许您为标记为 `inert` 属性的元素设置样式：
...
需要注意的是，由于生成的子选择器的特异性，直接在子元素上覆盖样式不会起作用：

<TipBad>{<>不适用，子元素不能覆盖自己的样式。</>}</TipBad>

```html
<!-- [!code classes:*:bg-sky-50] -->
<!-- [!code classes:bg-red-50] -->
<ul class="*:bg-sky-50 ...">
  <li class="bg-red-50 ...">销售</li>
  <li>市场营销</li>
  <li>搜索引擎优化</li>
  <!-- ... -->
</ul>
...
像 `*` 一样，`**` 变体可用于为元素的子元素设置样式。主要区别在于 `**` 将应用样式于 _所有_ 后代，而不仅仅是直接子元素。当您将其与其他变体结合以缩小选择范围时，这特别有用：
...
与 [任意值](/docs/adding-custom-styles#using-arbitrary-values) 允许您与工具类一起使用自定义值相同，任意变体允许您直接在 HTML 中编写自定义选择器变体。
...
任意变体可以与内置变体或相互叠加，就像 Tailwind 中的其他变体一样：
...
如果需要选择器中有空格，可以使用下划线。例如，此任意变体选择您添加了类的元素中的所有 `p` 元素：
...
您还可以在任意变体中使用 at-rule，例如 `@media` 或 `@supports`：
...
在使用 at-rule 自定义变体时，`&` 占位符不是必需的，就像与预处理器进行嵌套时一样。

### 注册自定义变体

如果您发现自己多次在项目中使用相同的任意变体，创建一个自定义变体可能是有意义的：
...
当自定义变体不需要嵌套时，可以使用简写语法：

```css
@custom-variant pointer-coarse (@media (pointer: coarse));
```

当自定义变体有多个规则时，可以将它们嵌套在彼此内部：
  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
          <code className="whitespace-nowrap before:content-none after:content-none">
            [dir="rtl"] <span className="text-gray-400">&</span>
          </code>
        </td>
      </tr>
      <tr>
        ...
        <td>
          <code className="whitespace-nowrap before:content-none after:content-none">
            [dir="ltr"] <span className="text-gray-400">&</span>
          </code>
        </td>
      </tr>
      <tr>
        ...
        <td>
          <code className="whitespace-nowrap before:content-none after:content-none">
            <span className="text-gray-400">&</span>:is([open], :popover-open)
          </code>
        </td>
      </tr>
      <tr>
...
这是 Tailwind 中包含的所有伪类变体的示例综合列表，以补充本指南开头的 [伪类文档](/docs/hover-focus-and-other-states#pseudo-classes)。
...
在元素或其后代获得焦点时，使用 `focus-within` 变体为元素设置样式：
...
在元素通过键盘获得焦点时，使用 `focus-visible` 变体为元素设置样式：
...
在元素的 ID 匹配当前 URL 片段时使用 `target` 变体为元素设置样式：
...
如果元素是第一个子元素，使用 `first` 变体为元素设置样式：
...
如果元素是最后一个子元素，使用 `last` 变体为元素设置样式：
...
如果元素是偶数编号子元素，使用 `even` 变体为元素设置样式：
...
如果元素是同类型的第一个子元素，使用 `first-of-type` 变体为元素设置样式：
...
如果元素是同类型的最后一个子元素，使用 `last-of-type` 变体为元素设置样式：
...
如果元素是同类型的唯一子元素，使用 `only-of-type` 变体为元素设置样式：
...
使用 `nth-last` 变体为特定位置的元素设置样式，从末尾计算：
...
使用 `nth-of-type` 变体为同类型的特定位置的元素设置样式：
...
使用 `nth-last-of-type` 变体为同类型的特定位置的元素设置样式，从末尾计算：
...
在输入元素被禁用时使用 `disabled` 变体为元素设置样式：
...
在输入元素被禁用时，仅在输入元素可用时使用 `enabled` 变体添加样式，这在您仅想在元素未禁用时应用其他样式时最有用：
...
在复选框或单选按钮被选中时使用 `checked` 变体为其设置样式：
...
使用 `indeterminate` 变体为复选框或单选按钮设置不确定状态样式：
...
使用 `default` 变体为初始加载页面时的默认值（选项、复选框或单选按钮）设置样式：
...
使用 `in-range` 变体为在指定范围限制内的输入设置样式：
...
使用 `out-of-range` 变体为超出指定范围限制的输入设置样式：
...
使用 `placeholder-shown` 变体为输入中显示占位符时的样式：
