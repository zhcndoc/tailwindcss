...
export const title = "指针事件";
export const description = "控制元素是否响应指针事件的工具类。";
...
使用 `pointer-events-none` 工具类使元素忽略指针事件，例如 `:hover` 和 `click` 事件：

<Figure hint="点击搜索图标查看预期效果">
...
指针事件仍然会在子元素上触发，并传递到“目标下方”的元素。

### 恢复指针事件

使用 `pointer-events-auto` 工具类恢复指针事件的默认浏览器行为：
