...
export const description = "控制在快照容器内项目周围的滚动偏移的工具。";
...
使用 `scroll-mt-<number>`、`scroll-mr-<number>`、`scroll-mb-<number>` 和 `scroll-ml-<number>` 工具，如 `scroll-ml-4` 和 `scroll-mt-6`，设置快照容器内项目周围的滚动偏移：
...
使用 `scroll-ms-<number>` 和 `scroll-me-<number>` 工具设置 `scroll-margin-inline-start` 和 `scroll-margin-inline-end` [逻辑属性](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts)，这些属性根据文本方向映射到左侧或右侧：
...
<Example padding={false}>
  {
    <>
      <p className="mb-4 pt-8 pl-6 text-sm font-medium">从左至右</p>
      ...
      <p className="mt-4 mb-4 pl-6 text-sm font-medium">从右至左</p>
...
为了获得更好的控制，还可以使用 [LTR 和 RTL 修饰符](/docs/hover-focus-and-other-states#rtl-support) 来根据当前文本方向有条件地应用特定样式。

### 使用 `scroll-mbs-<number>` 和 `scroll-mbe-<number>`

使用 `scroll-mbs-<number>` 和 `scroll-mbe-<number>` 工具设置 `scroll-margin-block-start` 和 `scroll-margin-block-end` 逻辑属性，这些属性根据书写模式映射到顶部或底部：
