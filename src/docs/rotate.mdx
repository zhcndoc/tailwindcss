...
使用 `rotate-<number>` 工具类，例如 `rotate-45` 和 `rotate-90`，以度为单位旋转一个元素：
...
使用 `-rotate-<number>` 工具类，例如 `-rotate-45` 和 `-rotate-90`，以度为单位逆时针旋转一个元素：
...
使用 `rotate-x-<number>`、`rotate-y-<number>` 和 `rotate-z-<number>` 工具类，例如 `rotate-x-50`、`-rotate-y-30` 和 `rotate-z-45` 共同旋转一个元素：
