...
export const description = "用于控制CSS过渡延迟的工具类。";
...
使用 `delay-150` 和 `delay-700` 等工具类来设置元素的过渡延迟（以毫秒为单位）：
...
在用户指定偏好减少动态效果的情况下，可以使用 `motion-safe` 和 `motion-reduce` 变体有条件地应用动画和过渡效果：
...
<UsingACustomValue element="button" utility="delay" value="1s,250ms" name="过渡延迟" />
