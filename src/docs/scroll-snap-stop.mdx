...
export const description = "用于控制是否可以跳过可能的快照位置的工具。";
...
将 `snap-always` 工具与 [snap-mandatory](/docs/scroll-snap-type#mandatory-scroll-snapping) 工具结合使用，以强制快照容器在用户继续滚动到下一个项目之前始终停在一个元素上：
...
使用 `snap-normal` 工具允许快照容器跳过可能的滚动快照位置：
