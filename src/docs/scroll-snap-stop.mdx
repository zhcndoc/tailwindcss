...
export const description = "控制是否可以跳过可能的捕捉位置的工具类。";
...
使用 `snap-always` 工具类与 [snap-mandatory](/docs/scroll-snap-type#mandatory-scroll-snapping) 工具类一起使用，以强制捕捉容器在用户继续滚动到下一个项目之前始终在一个元素上停止：
...
```html
<!-- [!代码 类别:snap-x,snap-mandatory,snap-always] -->
...
使用 `snap-normal` 工具类来允许捕捉容器跳过可能的滚动捕捉位置：
