...
export const title = "列数";
export const description = "用于控制元素内列数的工具。";
...
使用 `columns-<number>` 工具类，例如 `columns-3`，来设置内容在元素中应该创建的列数：
...
使用 `columns-xs` 和 `columns-sm` 等工具类来为元素中的内容设置理想的列宽：

<Figure desktopHint="调整示例大小以查看预期效果">
...
设置列宽时，列数会自动调整以确保它们不会变得过窄。

### 设置列间距

使用 `gap-<width>` 工具类来指定列之间的宽度：
...
了解更多关于间距工具的信息，请参阅 [间距文档](/docs/gap)。
