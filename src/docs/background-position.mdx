...
export const title = "背景位置";
export const description = "用于控制元素背景图像位置的工具类。";
...
使用 `bg-center`、`bg-right` 和 `bg-top-left` 等工具类来控制元素背景图像的位置：

<Figure hint="将鼠标悬停在这些示例上以查看完整图像。">
...
```html
<!-- [!代码类名：bg-top-left,bg-top-right,bg-bottom-right,bg-bottom-left,bg-center,bg-right,bg-top,bg-bottom,bg-left] -->
...
<UsingACustomValue utility="bg-position" value="center_top_1rem" name="背景位置" variable="bg-position" />
