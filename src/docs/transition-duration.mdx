...
export const description = "用于控制 CSS 过渡持续时间的工具。";
...
使用类似 `duration-150` 和 `duration-700` 的工具设置元素的过渡持续时间（以毫秒为单位）：

<Figure hint="鼠标悬停在每个按钮上以查看预期行为">
...
<UsingACustomValue element="button" utility="duration" value="1s,15s" name="过渡持续时间" />
