...
export const title = "背景图像";
export const description = "用于控制元素背景图像的工具类。";
...
使用像 `bg-linear-to-r` 和 `bg-linear-<angle>` 这样的工具类与 [颜色停止工具](#setting-gradient-color-stops) 一起为元素添加线性渐变：
...
使用 `bg-radial` 和 `bg-radial-[<position>]` 工具类与 [颜色停止工具](#setting-gradient-color-stops) 一起为元素添加放射状渐变：
...
使用 `bg-conic` 和 `bg-conic-<angle>` 工具类与 [颜色停止工具](#setting-gradient-color-stops) 一起为元素添加锥形渐变：
...
使用像 `from-indigo-500`、`via-purple-500` 和 `to-pink-500` 这样的工具类设置渐变停止的颜色：
...
使用像 `from-10%`、`via-30%` 和 `to-90%` 这样的工具类为渐变颜色停止设置更精确的位置：
...
<Example>
  {
    <div>
      <div className="grid grid-cols-1 items-center gap-x-4 gap-y-0 sm:grid-cols-[auto_1fr] sm:gap-y-2">
        <p className="mt-2 font-mono text-sm font-medium text-gray-500 sm:mt-0 sm:text-right dark:text-gray-400">
          srgb
        </p>
        <div className="mt-1 h-12 rounded-lg bg-linear-to-r/srgb from-indigo-500 to-teal-400"></div>
        <p className="mt-2 font-mono text-sm font-medium text-gray-500 sm:mt-0 sm:text-right dark:text-gray-400">hsl</p>
        ...
        <p className="mt-2 font-mono text-sm font-medium text-wrap text-gray-500 sm:mt-0 sm:text-right dark:text-gray-400">
          increasing
        </p>
...
默认情况下，渐变在 `oklab` 颜色空间中插值。

### 移除背景图像

使用 `bg-none` 工具类从元素中移除现有的背景图像：
