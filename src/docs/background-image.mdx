...
使用像 `bg-linear-to-r` 和 `bg-linear-<angle>` 的工具类与 [颜色停止工具](#setting-gradient-color-stops) 将线性渐变添加到元素：
...
使用 `bg-radial` 和 `bg-radial-[<position>]` 工具类与 [颜色停止工具](#setting-gradient-color-stops) 将径向渐变添加到元素：
...
使用 `bg-conic` 和 `bg-conic-<angle>` 工具类与 [颜色停止工具](#setting-gradient-color-stops) 将圆锥渐变添加到元素：
...
使用像 `from-indigo-500`、`via-purple-500` 和 `to-pink-500` 的工具类设置渐变停止的颜色：
...
使用像 `from-10%`、`via-30%` 和 `to-90%` 的工具类为渐变颜色停止设置更精确的位置：
