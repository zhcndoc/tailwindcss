import { ApiTable } from "@/components/api-table.tsx";
import { Example } from "@/components/example.tsx";
import { Figure } from "@/components/figure.tsx";
import { ResponsiveDesign, UsingACustomValue } from "@/components/content.tsx";

export const title = "列表样式类型";
export const description = "控制列表标记样式的工具类。";

<ApiTable
  rows={[
    ["list-disc", "list-style-type: disc;"],
    ["list-decimal", "list-style-type: decimal;"],
    ["list-none", "list-style-type: none;"],
    ["list-(<custom-property>)", "list-style-type: var(<custom-property>);"],
    ["list-[<value>]", "list-style-type: <value>;"],
  ]}
/>

## 示例

### 基本示例

使用 `list-disc` 和 `list-decimal` 等工具类来控制列表中标记的样式：

<Figure>

<Example>
  {
    <div className="flex flex-col gap-8">
      <div>
        <span className="mb-3 font-mono text-sm font-medium text-gray-500 dark:text-gray-400">list-disc</span>
        <ul className="list-inside list-disc text-gray-900 dark:text-gray-200">
          <li>现在这是一个关于我的生活被彻底颠覆的故事</li>
          <li>我想花一点时间让你坐在那儿</li>
          <li>我会告诉你我是如何成为贝尔艾尔镇的王子的</li>
        </ul>
      </div>
      <div>
        <span className="mb-3 font-mono text-sm font-medium text-gray-500 dark:text-gray-400">list-decimal</span>
        <ul className="list-inside list-decimal text-gray-900 dark:text-gray-200">
          <li>现在这是一个关于我的生活被彻底颠覆的故事</li>
          <li>我想花一点时间让你坐在那儿</li>
          <li>我会告诉你我是如何成为贝尔艾尔镇的王子的</li>
        </ul>
      </div>
      <div>
        <span className="mb-3 font-mono text-sm font-medium text-gray-500 dark:text-gray-400">list-none</span>
        <ul className="list-inside list-none text-gray-900 dark:text-gray-200">
          <li>现在这是一个关于我的生活被彻底颠覆的故事</li>
          <li>我想花一点时间让你坐在那儿</li>
          <li>我会告诉你我是如何成为贝尔艾尔镇的王子的</li>
        </ul>
      </div>
    </div>
  }
</Example>

```html
<!-- [!code classes:list-disc] -->
<ul class="list-disc">
  <li>现在这是一个关于我的生活被彻底颠覆的故事</li>
  <!-- ... -->
</ul>

<!-- [!code classes:list-decimal] -->
<ol class="list-decimal">
  <li>现在这是一个关于我的生活被彻底颠覆的故事</li>
  <!-- ... -->
</ol>

<!-- [!code classes:list-none] -->
<ul class="list-none">
  <li>现在这是一个关于我的生活被彻底颠覆的故事</li>
  <!-- ... -->
</ul>
```

</Figure>

### 使用自定义值

<UsingACustomValue element="ol" utility="list" name="marker" value="upper-roman" variable="marker" />

### 响应式设计

<ResponsiveDesign element="ul" property="list-style-type" defaultClass="list-none" featuredClass="list-disc" />
