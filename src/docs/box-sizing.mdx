...
export const description = "用于控制浏览器如何计算元素的总大小的工具。";
...
使用 `box-border` 工具将元素的 `box-sizing` 设置为 `border-box`，告知浏览器在您为其指定高度或宽度时，应包括元素的边框和内边距。

这意味着一个 100px &times; 100px 的元素，如果有 2px 的边框和 4px 的内边距，将会渲染为 100px &times; 100px，内部内容区域为 88px &times; 88px：
...
Tailwind 将此作为我们所有元素的默认设置，存在于 [preflight 基础样式](/docs/preflight)。

### 排除边框和内边距

使用 `box-content` 工具将元素的 `box-sizing` 设置为 `content-box`，告知浏览器在元素指定的宽度或高度上添加边框和内边距。

这意味着一个 100px &times; 100px 的元素，如果有 2px 的边框和 4px 的内边距，实际上会渲染为 112px &times; 112px，内部内容区域为 100px &times; 100px：
