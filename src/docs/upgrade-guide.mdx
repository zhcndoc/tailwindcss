...
在任意值中，`grid-cols-*`、`grid-rows-*` 和 `object-*` 工具之前会将逗号替换为空格。这种特殊行为在 Tailwind CSS v3 中是为了兼容 v2 而存在的。在 v4.0 中，这种兼容性已不再存在，必须使用下划线来表示空格。

要针对此更改更新您的项目，请将原本用作空格的逗号替换为下划线：
