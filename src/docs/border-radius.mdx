...
export const title = "边框圆角";
export const description = "用于控制元素的边框圆角的工具类.";
...
使用 `rounded-sm` 和 `rounded-md` 等工具类，为元素应用不同的边框圆角大小：
...
使用 `rounded-t-md` 和 `rounded-r-lg` 等工具类，只对元素的一侧进行圆角处理：
...
使用 `rounded-tr-md` 和 `rounded-tl-lg` 等工具类，仅对元素的一个角进行圆角处理：
...
使用 `rounded-s-md` 和 `rounded-se-xl` 等工具类，利用[逻辑属性](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts)设置边框圆角，这些属性根据文本方向映射到适当的角：
...
为了获得更多的控制，您还可以使用[LTR和RTL修饰符](/docs/hover-focus-and-other-states#rtl-support)根据当前文本方向条件应用特定样式。
...
<UsingACustomValue utility="rounded" name="边框圆角" value="2vw" variable="radius" />
...
<CustomizingYourTheme utility="rounded" themeKey="radius" name="边框圆角" customName="5xl" customValue="3rem" />