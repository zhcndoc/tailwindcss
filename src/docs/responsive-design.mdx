...
<!-- [!code word:md\:w-32] -->
<!-- [!code word:lg\:w-48] -->
<!-- 默认情况下宽度为 16，在中等屏幕上为 32，在大屏幕上为 48 -->
...
当您需要使用依赖于块尺寸的容器查询长度单位（如 `cqb`）时，请使用 `@container-size` 将元素标记为尺寸容器，而不是内联尺寸容器：
...
您也可以使用 `@container-size/{name}` 为尺寸容器命名。
...
使用 [容器查询长度单位](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_containment/Container_queries#container_query_length_units) 如 `cqw` 和 `cqi` 作为其他工具类中的任意值来引用容器大小：
...
对于需要超出内联尺寸的单位，如 `cqb` 和 `cqh`，请使用 `@container-size` 以便可以使用容器的完整尺寸。
