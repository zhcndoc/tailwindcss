...
export const title = "过滤器";
export const description = "用于为元素应用过滤器的工具。";
...
使用 `blur-xs` 和 `grayscale` 等工具为元素应用过滤器：
...
你可以组合以下过滤器工具： [blur](/docs/filter-blur), [brightness](/docs/filter-brightness), [contrast](/docs/filter-contrast), [drop-shadow](/docs/filter-drop-shadow), [grayscale](/docs/filter-grayscale), [hue-rotate](/docs/filter-hue-rotate), [invert](/docs/filter-invert), [saturate](/docs/filter-saturate), 和 [sepia](/docs/filter-sepia)。

### 移除过滤器

使用 `filter-none` 工具移除应用于元素的所有过滤器：
