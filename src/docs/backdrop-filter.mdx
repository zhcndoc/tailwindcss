import { ApiTable } from "@/components/api-table.tsx";
import { Example } from "@/components/example.tsx";
import { Figure } from "@/components/figure.tsx";
import { ResponsiveDesign, TargetingSpecificStates, UsingACustomValue } from "@/components/content.tsx";

export const title = "背景滤镜";
export const description = "用于为元素应用背景滤镜的工具类。";

<ApiTable
  rows={[
    ["backdrop-filter-none", "backdrop-filter: none;"],
    ["backdrop-filter-(<custom-property>)", "backdrop-filter: var(<custom-property>);"],
    ["backdrop-filter-[<value>]", "backdrop-filter: <value>;"],
  ]}
/>

## 示例

### 基本示例

使用 `backdrop-blur-xs` 和 `backdrop-grayscale` 等工具类为元素的背景应用滤镜：

<Figure>

<Example padding={false}>
  {
    <div className="flex scroll-p-8 justify-start overflow-scroll sm:block sm:overflow-visible">
      <div className="flex shrink-0 items-center justify-around gap-6 p-8 font-mono font-bold sm:gap-4">
        <div className="flex shrink-0 flex-col items-center">
          <p className="mb-3 text-center font-mono text-xs font-medium text-gray-500 dark:text-gray-400">
            背景模糊（极小）
          </p>
          <div className="relative">
            <div className="absolute inset-6 size-20 bg-white/30 backdrop-blur-xs"></div>
            <img
              className="size-32 rounded-lg object-cover"
              src="https://images.unsplash.com/photo-1554629947-334ff61d85dc?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=1000&h=1000&q=90"
            />
            <div className="absolute inset-0 rounded-lg ring-1 ring-black/10 ring-inset"></div>
          </div>
        </div>
        <div className="flex shrink-0 flex-col items-center">
          <p className="mb-3 text-center font-mono text-xs font-medium text-gray-500 dark:text-gray-400">
            背景灰度
          </p>
          <div className="relative">
            <div className="absolute inset-6 size-20 bg-white/30 backdrop-grayscale"></div>
            <img
              className="size-32 rounded-lg object-cover"
              src="https://images.unsplash.com/photo-1554629947-334ff61d85dc?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=1000&h=1000&q=90"
            />
            <div className="absolute inset-0 rounded-lg ring-1 ring-black/10 ring-inset"></div>
          </div>
        </div>
        <div className="flex shrink-0 flex-col items-center">
          <p className="mb-3 text-center font-mono text-xs font-medium text-gray-500 italic dark:text-gray-400">
            组合效果
          </p>
          <div className="relative">
            <div className="absolute inset-6 size-20 bg-white/30 backdrop-blur-xs backdrop-grayscale"></div>
            <img
              className="size-32 rounded-lg object-cover"
              src="https://images.unsplash.com/photo-1554629947-334ff61d85dc?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=1000&h=1000&q=90"
            />
            <div className="absolute inset-0 rounded-lg ring-1 ring-black/10 ring-inset"></div>
          </div>
        </div>
      </div>
    </div>
  }
</Example>

```html
<!-- [!code classes:背景模糊-xs,背景灰度] -->
<div class="bg-[url(/img/mountains.jpg)] ...">
  <div class="背景模糊-xs ..."></div>
</div>
<div class="bg-[url(/img/mountains.jpg)] ...">
  <div class="背景灰度 ..."></div>
</div>
<div class="bg-[url(/img/mountains.jpg)] ...">
  <div class="背景模糊-xs 背景灰度 ..."></div>
</div>
```

</Figure>

你可以组合以下背景滤镜工具类：[模糊](/docs/backdrop-filter-blur)、[亮度](/docs/backdrop-filter-brightness)、[对比度](/docs/backdrop-filter-contrast)、[灰度](/docs/backdrop-filter-grayscale)、[色相旋转](/docs/backdrop-filter-hue-rotate)、[反转](/docs/backdrop-filter-invert)、[不透明度](/docs/backdrop-filter-opacity)、[饱和度](/docs/backdrop-filter-saturate) 和 [怀旧](/docs/backdrop-filter-sepia)。

### 移除滤镜

使用 `背景滤镜-无` 工具类来移除应用于元素的所有背景滤镜：

```html
<!-- [!code classes:md:背景滤镜-无] -->
<div class="背景模糊-md 背景亮度-150 md:背景滤镜-无"></div>
```

### 使用自定义值

<UsingACustomValue utility="背景滤镜" value="url('filters.svg#filter-id')" name="背景滤镜" />

### 在悬停时应用

<TargetingSpecificStates
  property="背景滤镜"
  defaultClass="背景模糊-sm"
  featuredClass="背景滤镜-无"
/>

### 响应式设计

<ResponsiveDesign property="背景滤镜" defaultClass="背景模糊-sm" featuredClass="背景滤镜-无" />