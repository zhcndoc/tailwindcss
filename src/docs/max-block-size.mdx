...
export const description = "用于设置元素最大块尺寸的工具类。";
...
使用 `max-block-<number>` 工具类，如 `max-block-24` 和 `max-block-64`，根据间距刻度设置元素固定的最大块尺寸：
...
使用 `max-block-full` 或类似 `max-block-1/2`、`max-block-2/5` 的 `max-block-<fraction>` 工具类，为元素设置基于百分比的最大块尺寸：
...
<UsingACustomValue utility="max-block" value="220px" name="最大块尺寸" variable="max-block-size" />
