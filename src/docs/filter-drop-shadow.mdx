...
export const title = "滤镜: drop-shadow()";
export const description = "将 drop-shadow 滤镜应用于元素的工具类.";
...
使用像 `drop-shadow-sm` 和 `drop-shadow-xl` 的工具类为元素添加阴影:

<Figure>

<Example padding={false}>
  {
    <div className="flex scroll-p-8 justify-start overflow-scroll bg-white sm:block sm:overflow-visible">
      <div className="flex shrink-0 items-center justify-around gap-6 p-8 font-mono font-bold sm:gap-4">
        <div className="flex shrink-0 flex-col items-center">
          <p className="mb-3 text-center font-mono text-sm font-medium text-gray-500">drop-shadow-md</p>
        ...
        <div className="flex shrink-0 flex-col items-center">
          <p className="mb-3 text-center font-mono text-sm font-medium text-gray-500">drop-shadow-lg</p>
...
这对于将阴影应用于不规则形状（如文本和 SVG 元素）非常有用。如果要将阴影应用于规则元素，您可能希望使用 [盒子阴影](/docs/box-shadow) 。

### 使用自定义值

<UsingACustomValue element="svg" utility="drop-shadow" value="0_35px_35px_rgba(0,0,0,0.25)" name="投影阴影" />
