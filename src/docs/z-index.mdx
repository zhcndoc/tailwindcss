...
export const description = "控制元素堆叠顺序的工具类。";
...
使用 `z-<number>` 工具类，例如 `z-10` 和 `z-50`，以控制元素的堆叠顺序（或三维定位），无论它的显示顺序如何：
...
要使用负的 z-index 值，可以在类名前加上一个短横线，将其转换为负值：
...
<UsingACustomValue utility="z" name="堆叠顺序" value="calc(var(--index)+1)" />
