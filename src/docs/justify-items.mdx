...
export const description = "用于控制网格项目在其行轴上对齐方式的工具。";
...
使用 `justify-items-start` 工具将网格项目对齐到其行轴的起始位置：
...
使用 `justify-items-end` 或 `justify-items-end-safe` 工具将网格项目对齐到其行轴的结束位置：
...
当可用空间不足时，`justify-items-end-safe` 工具将对齐项目到容器的起始位置，而不是结束位置。

### 居中

使用 `justify-items-center` 或 `justify-items-center-safe` 工具将网格项目对齐到其行轴的中心位置：
...
当可用空间不足时，`justify-items-center-safe` 工具将对齐项目到容器的起始位置，而不是中心。

### 拉伸

使用 `justify-items-stretch` 工具在其行轴上拉伸项目：
