...
使用 `p-<number>` 工具类，如 `p-4` 和 `p-8` 来控制元素四周的内边距：
...
使用 `pt-<number>`、`pr-<number>`、`pb-<number>` 和 `pl-<number>` 工具类，如 `pt-6` 和 `pr-4` 来控制元素一侧的内边距：
...
使用 `px-<number>` 工具类，如 `px-4` 和 `px-8` 来控制元素的水平内边距：
...
使用 `py-<number>` 工具类，如 `py-4` 和 `py-8` 来控制元素的垂直内边距：
...
使用 `ps-<number>` 或 `pe-<number>` 工具类，如 `ps-4` 和 `pe-8` 来设置 `padding-inline-start` 和 `padding-inline-end` 逻辑属性，这些逻辑属性根据文本方向映射到左侧或右侧：
...
为了获得更多的控制，你还可以使用 [LTR 和 RTL 修饰符](/docs/hover-focus-and-other-states#rtl-support) 根据当前文本方向有条件地应用特定样式。
