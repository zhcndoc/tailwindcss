...
export const title = "宽度";
export const description = "用于设置元素宽度的工具。";
...
使用 `w-<number>` 实用工具，如 `w-24` 和 `w-64` 来根据间距比例设置元素的固定宽度：
...
使用 `w-full` 或 `w-<fraction>` 实用工具，如 `w-1/2` 和 `w-2/5` 给元素设置基于百分比的宽度：
...
使用 `w-sm` 和 `w-xl` 等工具，根据容器比例设置元素的固定宽度：
...
另外，您可以使用 `w-lvw`、`w-svw` 和 `w-dvw` 实用工具来匹配大、小或动态视口的宽度。

### 重置宽度

使用 `w-auto` 实用工具在特定条件下移除元素的指定宽度，例如在特定的断点处：
...
使用 `size-px`、`size-4` 和 `size-full` 等实用工具可以同时设置元素的宽度和高度：
