...
export const description = "用于设置元素宽度的工具类。";
...
使用 `w-<number>` 工具类，如 `w-24` 和 `w-64`，根据间距比例设置元素的固定宽度：
...
使用 `w-full` 或 `w-<fraction>` 工具类，如 `w-1/2` 和 `w-2/5`，为元素设置基于百分比的宽度：
...
使用 `w-sm` 和 `w-xl` 等工具类，根据容器比例设置元素的固定宽度：
...
您也可以使用 `w-lvw`、`w-svw` 和 `w-dvw` 工具类匹配大、小或动态视口的宽度。

### 重置宽度

使用 `w-auto` 工具类在特定条件下移除元素的指定宽度，例如在特定断点时：
...
使用如 `size-px`、`size-4` 和 `size-full` 的工具类，同时设置元素的宽度和高度：
...
<UsingACustomValue utility="w" value="5px" name="宽度" variable="width" />

### 响应式设计

<ResponsiveDesign property="宽度" defaultClass="w-1/2" featuredClass="w-full" />
