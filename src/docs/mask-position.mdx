...
export const description = "用于控制元素的遮罩图像位置的工具类。";
...
使用 `mask-center`、`mask-right` 和 `mask-left-top` 工具类来控制元素遮罩图像的位置：
...
<UsingACustomValue utility="mask-position" value="center_top_1rem" name="遮罩位置" variable="mask-position" />
