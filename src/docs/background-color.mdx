import { ApiTable } from "@/components/api-table.tsx";
import {
  CustomizingYourThemeColors,
  ResponsiveDesign,
  TargetingSpecificStates,
  UsingACustomValue,
} from "@/components/content.tsx";
import { Example } from "@/components/example.tsx";
import { Figure } from "@/components/figure.tsx";
import colors from "./utils/colors";

export const title = "背景颜色";
export const description = "用于控制元素背景颜色的工具类。";

<ApiTable
  rows={[
    ["bg-inherit", "background-color: inherit;"],
    ["bg-current", "background-color: currentColor;"],
    ["bg-transparent", "background-color: transparent;"],
    ...Object.entries(colors).map(([name, value]) => [
      `bg-${name}`,
      `background-color: var(--color-${name}); /* ${value} */`,
    ]),
    ["bg-(<custom-property>)", "background-color: var(<custom-property>);"],
    ["bg-[<value>]", "background-color: <value>;"],
  ]}
/>

## 示例

### 基本示例

使用类似 `bg-white`、`bg-indigo-500` 和 `bg-transparent` 的工具类来控制元素的背景颜色：

<Figure>

<Example>
  {
    <div className="grid grid-cols-1 gap-4 text-sm text-white sm:grid-cols-3">
      <div className="flex shrink-0 flex-col items-center">
        <p className="mb-3 text-center font-mono text-sm font-medium text-gray-500 dark:text-gray-400">bg-blue-500</p>
        <button
          type="button"
          tabIndex="-1"
          className="rounded-md bg-blue-500 px-4 py-2 text-sm font-semibold text-white opacity-100 focus:outline-none"
        >
          按钮 A
        </button>
      </div>
      <div className="flex shrink-0 flex-col items-center">
        <p className="mb-3 text-center font-mono text-sm font-medium text-gray-500 dark:text-gray-400">bg-cyan-500</p>
        <button
          type="button"
          tabIndex="-1"
          className="rounded-md bg-cyan-500 px-4 py-2 text-sm font-semibold text-white opacity-100 focus:outline-none"
        >
          按钮 B
        </button>
      </div>
      <div className="flex shrink-0 flex-col items-center">
        <p className="mb-3 text-center font-mono text-sm font-medium text-gray-500 dark:text-gray-400">bg-pink-500</p>
        <button
          type="button"
          tabIndex="-1"
          className="rounded-md bg-pink-500 px-4 py-2 text-sm font-semibold text-white opacity-100 focus:outline-none"
        >
          按钮 C
        </button>
      </div>
    </div>
  }
</Example>

```html
<!-- [!code classes:bg-blue-500,bg-cyan-500,bg-pink-500] -->
<button class="bg-blue-500 ...">按钮 A</button>
<button class="bg-cyan-500 ...">按钮 B</button>
<button class="bg-pink-500 ...">按钮 C</button>
```

</Figure>

### 改变透明度

使用颜色透明度修饰符来控制元素背景颜色的透明度：

<Figure>

<Example>
  {
    <div className="grid grid-cols-1 gap-4 text-sm text-white sm:grid-cols-3">
      <div className="flex shrink-0 flex-col items-center">
        <p className="mb-3 text-center font-mono text-sm font-medium text-gray-500 dark:text-gray-400">
          bg-sky-500/100
        </p>
        <button
          type="button"
          tabIndex="-1"
          className="rounded-md bg-sky-500/100 px-4 py-2 text-sm font-semibold text-white opacity-100 focus:outline-none"
        >
          按钮 A
        </button>
      </div>
      <div className="flex shrink-0 flex-col items-center">
        <p className="mb-3 text-center font-mono text-sm font-medium text-gray-500 dark:text-gray-400">bg-sky-500/75</p>
        <button
          type="button"
          tabIndex="-1"
          className="rounded-md bg-sky-500/75 px-4 py-2 text-sm font-semibold text-white opacity-100 focus:outline-none"
        >
          按钮 B
        </button>
      </div>
      <div className="flex shrink-0 flex-col items-center">
        <p className="mb-3 text-center font-mono text-sm font-medium text-gray-500 dark:text-gray-400">bg-sky-500/50</p>
        <button
          type="button"
          tabIndex="-1"
          className="rounded-md bg-sky-500/50 px-4 py-2 text-sm font-semibold text-white opacity-100 focus:outline-none"
        >
          按钮 C
        </button>
      </div>
    </div>
  }
</Example>

```html
<!-- [!code word:/100] -->
<!-- [!code word:/75] -->
<!-- [!code word:/50] -->
<button class="bg-sky-500/100 ..."></button>
<button class="bg-sky-500/75 ..."></button>
<button class="bg-sky-500/50 ..."></button>
```

</Figure>

### 使用自定义值

<UsingACustomValue utility="bg" value="#50d71e" name="背景颜色" variable="color" />

### 鼠标悬停时应用

<TargetingSpecificStates property="background-color">

<Figure>

<Example>
  {
    <button
      type="button"
      tabIndex="-1"
      className="mx-auto block rounded-md bg-indigo-500 px-3 py-2 text-sm font-semibold text-white hover:bg-fuchsia-500 focus:outline-none"
    >
      保存更改
    </button>
  }
</Example>

```html
<!-- [!code classes:hover:bg-fuchsia-500] -->
<button class="bg-indigo-500 hover:bg-fuchsia-500 ...">保存更改</button>
```

</Figure>

</TargetingSpecificStates>

### 响应式设计

<ResponsiveDesign property="background-color" defaultClass="bg-blue-500" featuredClass="bg-green-500" />

## 自定义你的主题

<CustomizingYourThemeColors utility="bg" />
