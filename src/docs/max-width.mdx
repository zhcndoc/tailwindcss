...
使用 `max-w-<number>` 工具类，如 `max-w-24` 和 `max-w-64`，根据间距比例设置元素的固定最大宽度：
...
使用 `max-w-full` 或 `max-w-<fraction>` 工具类，如 `max-w-1/2` 和 `max-w-2/5`，为元素设置基于百分比的最大宽度：
...
使用工具类，如 `max-w-sm` 和 `max-w-xl`，根据容器比例设置元素的固定最大宽度：
...
使用 `container` 工具类将元素的最大宽度设置为当前断点的最小宽度。当您更喜欢为固定的一组屏幕尺寸设计而不是试图适应完全流式的视口时，这非常有用：
...
注意，与您可能在其他框架中使用的容器不同，Tailwind 的容器不会自动居中，并且没有内置的水平填充。使用 `mx-auto` 和 `px-<number>` 工具类添加这些：
