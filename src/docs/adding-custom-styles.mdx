...
如果你需要同时应用多个变体，请使用与 HTML 中相同的语法将它们堆叠起来：
...
可用的基本值数据类型有：`number`、`integer`、`ratio` 和 `percentage`。
...
这将匹配像 `tab-[1]` 和 `tab-[76]` 这样的工具。

可用的任意值数据类型有：`absolute-size`、`angle`、`bg-size`、`color`、`family-name`、`generic-name`、`image`、`integer`、`length`、`line-width`、`number`、`percentage`、`position`、`ratio`、`relative-size`、`url`、`vector` 和 `*`。
...
在 `--value()` 内使用 `--default()`，以支持在未提供显式值时使用默认值：
...
这会同时匹配 `tab-2` 和 `tab-4`，以及仅使用默认值 `4` 的 `tab`：
...
`--default()` 函数也可以在 `--modifier()` 内使用，以便在没有修饰符时提供默认值：
...
这会匹配 `tab-2/3`，其 line-height 为 `3`，以及 `tab-2`，其 line-height 为 `1`。
