...
使用 `invisible` 工具类隐藏元素，但仍然保持其在文档中的位置，影响其他元素的布局：
...
要完全从文档中移除一个元素，请使用 [display](/docs/display#hidden) 属性。

### 折叠元素

使用 `collapse` 工具类隐藏表格行、行组、列和列组，效果与设置为 `display: none` 相似，但不影响其他行和列的大小：
