...
export const description = "用于设置元素最小高度的工具类。";
...
使用 `min-h-<number>` 工具类，如 `min-h-24` 和 `min-h-64`，将元素设置为基于间距比例的固定最小高度：
...
使用 `min-h-full` 或者 `min-h-<fraction>` 工具类，如 `min-h-1/2` 和 `min-h-2/5` 来设置元素的基于百分比的最小高度：
