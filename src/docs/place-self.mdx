...
使用 `place-self-auto` 根据容器的 `place-items` 属性值对齐项目：
...
使用 `place-self-start` 在两个轴上将项目对齐到起始位置：
...
使用 `place-self-center` 在两个轴上将项目对齐到中心：
...
使用 `place-self-end` 在两个轴上将项目对齐到结束位置：
