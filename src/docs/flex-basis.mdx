...
export const description = "用于控制 flex 项目的初始大小的工具类。";
...
使用 `basis-<number>` 工具类，如 `basis-64` 和 `basis-128`，根据间距比例设置 flex 项目的初始大小：
...
使用工具类，如 `basis-xs` 和 `basis-sm`，根据容器比例设置 flex 项目的初始大小：
...
使用 `basis-<fraction>` 工具类，如 `basis-1/2` 和 `basis-2/3`，设置 flex 项目的初始大小：
