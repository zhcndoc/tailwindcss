...
export const title = "mask-mode"; // 遮罩模式
export const description = "用于控制元素的遮罩模式的工具."; // 用于控制元素的遮罩模式的工具
...
使用 `mask-alpha`、`mask-luminance` 和 `mask-match` 工具来控制元素的遮罩模式：
...
在使用 `mask-luminance` 时，遮罩的亮度值决定了可见性，因此使用灰度颜色将产生最可预测的结果。而在 `mask-alpha` 中，遮罩的透明度决定了遮罩元素的可见性。
