...
export const description = "控制数字变体的实用工具。";
...
使用 `ordinal` 实用工具在支持的字体中启用序数标记的特殊字形：
...
使用 `slashed-zero` 实用工具在支持的字体中强制使用带斜杠的零：
...
使用 `lining-nums` 实用工具在支持的字体中使用基线对齐的数字字形：
...
使用 `oldstyle-nums` 实用工具在支持的字体中使用某些数字有下沉部分的数字字形：
...
使用 `proportional-nums` 实用工具在支持的字体中使用具有比例宽度的数字字形：
...
使用 `tabular-nums` 实用工具在支持的字体中使用具有统一/表格宽度的数字字形：
...
使用 `diagonal-fractions` 实用工具在支持的字体中用常见的对角分数替换用斜杠分开的数字：
...
使用 `stacked-fractions` 实用工具在支持的字体中用常见的叠加分数替换用斜杠分开的数字：
...
`font-variant-numeric` 实用工具是可组合的，因此您可以通过组合它们启用多个变体：
