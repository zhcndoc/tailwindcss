...
使用像 `border-rose-500` 和 `border-lime-100` 的工具类来控制元素的边框颜色：
...
使用像 `border-t-indigo-500` 和 `border-r-lime-100` 的工具类来设置元素一侧的边框颜色：
...
使用像 `border-x-indigo-500` 和 `border-y-lime-100` 的工具类来同时设置元素两侧的边框颜色：
...
使用像 `border-s-indigo-500` 和 `border-e-lime-100` 的工具类来设置 `border-inline-start-color` 和 `border-inline-end-color` [逻辑属性](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts)，这些属性根据文本方向映射到左或右边框：
...
使用像 `divide-indigo-500` 和 `divide-lime-100` 的工具类来控制子元素之间的边框颜色：
...
<Example>
  {
    <label className="mx-auto block max-w-xs">
      <span className="text-sm font-medium text-gray-900 dark:text-gray-200">电子邮箱地址</span>
      <input
        type="text"
