...
export const title = "边框颜色";
export const description = "用于控制元素边框颜色的工具类。";
...
使用 `border-t-indigo-500` 和 `border-r-lime-100` 等工具类来设置元素单边的边框颜色：
...
使用 `border-x-indigo-500` 和 `border-y-lime-100` 等工具类来同时设置元素两侧的边框颜色：
...
使用 `border-s-indigo-500` 和 `border-e-lime-100` 等工具类设置 `border-inline-start-color` 和 `border-inline-end-color` [逻辑属性](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts)，这些属性会根据文本方向映射到左边或右边的边框：
...
### 使用 `border-bs-*` 和 `border-be-*` 工具类

使用 `border-bs-*` 和 `border-be-*` 工具类来设置 `border-block-start-color` 和 `border-block-end-color` 逻辑属性，这些属性会根据书写模式映射到顶部或底部边框：
...
<UsingACustomValue utility="border" name="边框颜色" value="#243c5a" />
...
<Example>
  {
    <label className="mx-auto block max-w-xs">
      <span className="text-sm font-medium text-gray-900 dark:text-gray-200">电子邮件地址</span>
      <input
        type="text"
