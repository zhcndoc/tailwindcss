...
export const description = "设置元素最大高度的工具类。";
...
使用 `max-h-<number>` 工具类，如 `max-h-24` 和 `max-h-64`，将元素设置为基于间距尺度的固定最大高度：
...
使用 `max-h-full` 或 `max-h-<fraction>` 工具类，如 `max-h-1/2` 和 `max-h-2/5` 为元素提供基于百分比的最大高度：
