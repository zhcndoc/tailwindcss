...
export const title = "最大高度";
export const description = "用于设置元素最大高度的工具类。";
...
使用 `max-h-<number>` 工具类，比如 `max-h-24` 和 `max-h-64` 来设置元素的固定最大高度，基于间距比例：
...
使用 `max-h-full` 或 `max-h-<fraction>` 工具类，如 `max-h-1/2` 和 `max-h-2/5` 为元素设置基于百分比的最大高度：
...
<UsingACustomValue utility="max-h" value="220px" name="最大高度" variable="max-height" />
