...
export const description = "用于控制哪些 CSS 属性过渡的工具类。";
...
使用 `transition` 和 `transition-colors` 等工具类来指定当属性变化时应进行过渡的属性：

<Figure hint="悬停在按钮上可以查看预期效果">
...
对于用户指定希望减少动画的情况，可以使用 `motion-safe` 和 `motion-reduce` 变体有条件地应用动画和过渡：
