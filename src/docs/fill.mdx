...
export const title = "填充";
export const description = "用于样式化 SVG 元素填充的工具类。";
...
使用像 `fill-indigo-500` 和 `fill-lime-600` 这样的工具类来改变 SVG 的填充颜色：
...
这对于样式化图标集如 [Heroicons](https://heroicons.com) 非常有用。

### 使用当前颜色

使用 `fill-current` 工具类将填充颜色设置为当前文本颜色：

<Figure hint="将鼠标悬停在按钮上以查看填充颜色变化">
...
<UsingACustomValue element="svg" utility="fill" name="填充颜色" value="#243c5a" variable="fill-color" />
