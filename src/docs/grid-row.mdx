...
export const description = "用于控制元素在网格行中大小和位置的工具类。";
...
使用 `row-span-<number>` 工具类，如 `row-span-2` 和 `row-span-4`，使元素跨越 _n_ 行：
...
使用 `row-start-<number>` 或 `row-end-<number>` 工具类，如 `row-start-2` 和 `row-end-3`，使元素在第 _n_ 个网格线上开始或结束：
...
这些也可以与 `row-span-<number>` 工具类结合使用，以跨越特定数量的行。
