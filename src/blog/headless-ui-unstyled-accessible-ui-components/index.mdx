...
**[无样式的 UI](https://headlessui.dev) 是一组完全无样式、完全可访问的 UI 组件，适用于 React 和 Vue** _(并即将支持 Alpine.js)_，使您能够轻松构建这些自定义组件，而无需担心任何复杂的实现细节，并且仍然能够使用简单的实用工具类从头开始进行样式设置。

<Image alt="无样式的 UI 标志" src={headlessUiCard} />
...
<iframe
  ...
  className="w-full overflow-hidden rounded border-0"
  style={{ height: "480px" }}
  title="@headlessui/react 菜单示例"
